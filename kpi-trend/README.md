...
Если вы клонировали весь репозиторий `Ad_Robot`, перейдите в каталог проекта:

```bash
cd kpi-trend
```

Дальше команды ниже предполагают, что текущая директория — `kpi-trend/`.
...
3. Проверьте, что в коммит не попали `node_modules`, `.next` и `.venv` — они уже в `.gitignore`.

## Заметки по проверке стилей

Визуальная сверка делалась объективно, а не «на глаз»: скриншоты обоих приложений
(Nginx/Vite-версия из zip-архива и эта) снимались в headless Chromium, после чего
в PIL считалась гистограмма насыщенных цветов и сравнивалась с кадрами референсного GIF.

Результат: эта версия попадает во все измеренные цвета референса
(`#FCE0E3`, `#E5F0FC`, `#FCECBB`, `#FCF893`, `#3670FC`, `#118603`, `#60BB21`, `#B601FC`),
а zip-версия — нет (у неё `#4C6FE0` вместо `#3670FC`, `#F2B705` вместо палево-жёлтого,
`#8CE83B` вместо `#60BB21`, серый фон `#F4F5F7` вместо голубого `#E5F0FC`).
